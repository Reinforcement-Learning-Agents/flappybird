...
class Config:
    ...
    max_steps: int = 600_000  # totale step ambiente
    max_episode_steps: int | None = None  # se vuoi un cap, metti un int
    ...
    eps_start: float = 1.0
    eps_end: float = 0.01
    eps_decay_steps: int = 400_000  # decay esponenziale su questi step
...
def train_step(qnet, qtarget, optim, rb: ReplayBuffer):
    ...
    with torch.no_grad():
        q_next = qtarget(s2_t).max(dim=1).values   # <-- DQN classico
...
def save_plot(eval_steps, eval_scores, out_png):
    ...
    plt.title("DQN FlappyBird (DQN + MSE_loss)")
